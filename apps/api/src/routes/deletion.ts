...
delection.post("/matters/:matterId/deletion", async (c) => {
  const matterId = c.req.param("matterId"); const user = c.get("user"); await requireMatterRole(c.env, c.get("accessToken"), user.id, matterId, "matter_manager");
  const body = await readJson<{ confirmation?: string; reason?: string }>(c.req.raw); if (body.confirmation !== matterId) throw new ApiError(400, "confirmation must exactly match the matter ID", "confirmation_required");
  const matter = (await serviceRest(c.env, `/matters?id=eq.${matterId}&select=id,organization_id,status&limit=1`))?.[0]; if (!matter || matter.status !== "active") throw new ApiError(409, "Matter cannot enter deletion", "invalid_matter_state");
  ...
  const jobId = crypto.randomUUID(); const { token } = await createJobEnvelope({ jobId, organizationId: matter.organization_id, matterId, stage: "delete_matter", pipelineVersion: "phase1.0.0", actorId: user.id, ttlSeconds: 86400 }, c.env.TOKEN_SIGNING_SECRET);
