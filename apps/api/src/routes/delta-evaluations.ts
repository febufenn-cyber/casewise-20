...
deltaEvaluations.post('/matters/:matterId/delta-evaluations', async (c) => {
  ...
  let value;
  try { value = validateDeltaEvaluationInput(body); }
  ...
  const deltaItemIds = [...new Set(value.observations.map((item) => item.delta_item_id).filter(Boolean))] as string[];
  ...
  await serviceRest(c.env, '/delta_evaluation_items', { method: 'POST', prefer: 'return=minimal', body: JSON.stringify(value.observations.map((item) => ({ id: crypto.randomUUID(), organization_id: matter.organization_id, matter_id: matterId, evaluation_run_id: runId, ...item }))) });
