...
Casewise is being designed to convert litigation and arbitration case bundles into a lawyer-reviewable map of documents, parties, events, allegations, responses, evidence, contradictions, unresolved questions, reviewed matter overviews and attorney-controlled work plans. Every material factual statement must resolve to the uploaded source or be marked unsupported, inferred, contradicted or uncertain.

## Current status

**Phases 0–5 are implemented in the repository. Production deployment, independent security verification, legal-domain evaluation, authority-source approval and design-partner evidence remain pending.**

The implemented layers cover the product constitution, secure evidence custody, document and chronology intelligence, allegation-response-evidence mapping, filing-to-filing deltas, matter memory, source-bound matter overviews, attorney-controlled response planning, exact-version approval, stale propagation, internal-only exports and executable evaluation gates. Repository completion does not authorize confidential production use or establish legal accuracy.
...
- **First job:** turn an English-language case bundle into a source-linked matter map and reviewed internal work plan.
- **First work products:** document register, party map, chronology, allegation-response matrix, contradiction register, missing-information register, internal matter overview and response-planning workspace.
...
The structured matter graph is the system of record. Generated prose and planning artifacts are versioned, reviewable views derived from that graph.

## Repository map

- [`docs/phase-0/`](docs/phase-0/) — constitution, scope, trust rules, economics, pilot design and merge gates.
- [`docs/phase-1/`](docs/phase-1/) through [`docs/phase-5/`](docs/phase-5/) — implemented phase contracts, handoffs and exit reviews.
- [`docs/roadmap/remaining-phases-autonomous-build-plan.md`](docs/roadmap/remaining-phases-autonomous-build-plan.md) — autonomous implementation protocol.
...
6. **Phase 5:** reviewed matter overview and attorney-controlled response planning — implemented.
7. **Phase 6:** verified legal authority research — next.
...
Three core phases remain before the controlled pilot-ready v1 finish line. Saying `build` instructs the implementation agent to verify and implement Phase 6 end-to-end under the roadmap contract, including green CI, squash merges, remote `main` verification and exact commit confirmation.
