...
for (const file of files) {
  ...
  for (const match of sql.matchAll(/create table public\.([a-z_]+)/gi)) tables.push(match[1]);
  for (const match of sql.matchAll(/alter table public\.([a-z_]+) enable row level security/gi)) rls.add(match[1]);
